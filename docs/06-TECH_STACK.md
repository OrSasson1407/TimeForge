...
- The app ships with no seed/demo data. A deployment's first Administrator account (and nothing else) is created via `scripts/create_admin.py`, which works against either the emulator or a real Firebase project — it's the caller's environment configuration that decides which, not the script.
